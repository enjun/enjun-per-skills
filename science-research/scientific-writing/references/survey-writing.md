...
Use the rubric in `evaluation-rubric.md` to score the paper:
...
Use the survey template `assets/main-survey.tex` (copy it to `latex/main.tex`) together with the shared IEEE files in `assets/`:
- `IEEEtran.cls` — IEEE document class
- `IEEEabrv.bib` — IEEE abbreviation database
- `IEEEtran.bst` — IEEE bibliography style
- `main-survey.tex` — Survey template main file
- `myref-survey.bib` — Example references database
